...
class AppDelegate: UIResponder, UIApplicationDelegate,RevMobAdsDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        if firstLaunch  {
            ...
            NSUserDefaults.standardUserDefaults().setBool(false, forKey: "isPurchased")
        ...
        if(NSUserDefaults.standardUserDefaults().boolForKey("isPurchased") == false) {
            if(showRevMob == true) {
                RevMobAds.startSessionWithAppID(kRevmobID, andDelegate: self);
            }
            else if(showChartboost == true) {
                Chartboost.startWithAppId(kChartboostAppID, appSignature: kChartboostAppSignature, delegate: nil);
                Chartboost.showInterstitial(CBLocationHomeScreen)
            }
        }
        
        GamecenterUtils.sharedGamecenterUtils.authenticateLocalUserOnViewController(window!.rootViewController!)
        return true
    }
